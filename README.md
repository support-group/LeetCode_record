# LeetCode_record
LeetCode刷题记录整合，包含题目描述、解题思路和解题代码

## 题目分类
### 1.链表_v1.0
[Leetcode  206.链表反转](https://leetcode.cn/problems/reverse-linked-list/)

[Leetcode  203.移除链表中的元素](https://leetcode.cn/problems/remove-linked-list-elements/)

[Leetcode  19.删除链表中的倒数第n个节点](https://leetcode.cn/problems/remove-nth-node-from-end-of-list/)

[Leetcode  24.两两交换链表的节点](https://leetcode.cn/problems/swap-nodes-in-pairs/)

[Leetcode  141.环形链表](https://leetcode.cn/problems/linked-list-cycle/ )

[Leetcode  142.环形链表II](https://leetcode.cn/problems/linked-list-cycle-ii/)


### 2.二分查找
[寻找峰值](https://www.baidu.com)

[搜索旋转数组](https://www.baidu.com)
### 3.双指针
[盛水最多的容器](https://www.baidu.com)
